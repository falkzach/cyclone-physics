...
class BallisticDemo : public Application
{
    ...
    cyclone::Vector3 point = cyclone::Vector3(0.0, 11.5, 5.0);
    cyclone::ParticleFakeSpring ppg = cyclone::ParticleFakeSpring(&point, 150.0, 1.0);
...
void BallisticDemo::fire()
{
    ...
    case FIREBALL:
        ...
        shot->particle.setVelocity(0.0f, 0.0f, 0.0f); // 5m/s
...
void BallisticDemo::update()
{
    ...
    for (AmmoRound *shot = ammo; shot < ammo+ammoRounds; shot++)
    {
        cyclone::Particle *p = &shot->particle;
        if (shot->type != UNUSED)
        {
            ppg.updateForce(p, duration);
            // Run the physics
            shot->particle.integrate(duration);

            cyclone::Vector3 velocity = shot->particle.getVelocity();
            std::cout << "particle velocity: " << velocity.x << ", " << velocity.y << ", " << velocity.z << std::endl;

            // Check if the particle is now invalid
            if (shot->particle.getPosition().y < 0.0f ||
                shot->startTime+50000 < TimingData::get().lastFrameTimestamp ||
