...
struct FireworkRule
{
    ...
    void create(Firework *firework, const Firework *parent = NULL) const
    {
        ...
        cyclone::Vector3 position = firework->getPosition();
        std::cout << "firework at (x,y): " << position[0] << "," << position[1] << std::endl;
    }
};
...
public:
    ...
    virtual void checkAll(float x, float y);
...
void FireworksDemo::initFireworkRules()
{
    ...
    rules[2].setParameters(
        ...
        cyclone::Vector3(28, 8, 28), // max velocity
        0.1 // damping
        );
...
{
    return "Cyclone > Fireworks Project";
}
...
void FireworksDemo::update()
{
         ...
         firework++)
    {
        ...
        if (firework->type > 0)
        {
            ...
            if (firework->update(duration) || firework->clicked)
            {
...
void FireworksDemo::mouse(int button, int state, int x, int y)
{
    if (state == GLUT_DOWN) {
        //look head on...
        gluLookAt(0.0, 0.0, 10.0, 0.0, 3.5, 0.0,  0.0, 1.0, 0.0);

        float world_x, world_y, scale_x, scale_y;

        world_x = ( (float) x - (0.5f * (float) width) );
        world_y = (float) height - (float) y;

        scale_x = (20.0f * world_x )/ (float) width;
        scale_y = (10.0f * world_y )/ (float) height;

        // std::cout << "mouse click at (x,y): " << x << "," << y << std::endl;
        // std::cout << "window size (width, height): " << width << "," << height << std::endl;
        // std::cout << "world click at (x,y): " << world_x << "," << world_y << std::endl;
        // std::cout << "world scale click at (x,y): " << scale_x << "," << scale_y << std::endl;

        fire(scale_x, scale_y);
    }  
}
...
void FireworksDemo::fire(float x, float y)
{
    float distance = 0.0f;
    for (Firework *firework = fireworks;
     firework < fireworks+maxFireworks;
    ...
    {
        if (firework->type != 0) {
            distance = sqrt(pow((x - firework->getPosition().x), 2.0) + pow((y - firework->getPosition().y), 2.0));
            if (distance <= 1.0f) {
                std::cout << "HIT!" << std::endl;
                firework->clicked = true;
            }
        }
    }
}

void FireworksDemo::checkAll(float x, float y){
    //TODO: OCT TREE MOFUCKER
}
